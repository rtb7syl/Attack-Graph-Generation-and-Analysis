...
const connectionString = 'postgres://ubuntu:'+ pgsql_passkey +'@localhost:5432/gosqldb'
...
/*client.query('CREATE TABLE hosts_connections(from_host VARCHAR(40) not null, to_host VARCHAR(40) not null)',function(err,res){
    if(err){
        console.log(err)
    }
    else{
    ...
    if(err){
		...
		for(var i = 0; i < linkDataArray.length; i++){
			...
			if(from.slice(0,4) == 'Host' && to.slice(0,4) == 'Host'){
				...
				var text = 'INSERT INTO hosts_connections(from_host, to_host) VALUES($1, $2)'
	            var values = [from, to]
